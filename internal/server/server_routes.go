...
func (s *Server) registerAdminRoutes(mux *http.ServeMux) {
	...
	mux.Handle("GET /admin/settings", admin(s.handleAdminSettings))
	mux.Handle("PATCH /admin/settings", admin(s.handleAdminUpdateSettings))
	mux.Handle("POST /admin/integrations", admin(s.handleAdminCreateIntegration))
	mux.Handle("PATCH /admin/integrations/{id}", admin(s.handleAdminUpdateIntegration))
	mux.Handle("POST /admin/integrations/{id}/test", admin(s.handleAdminTestIntegration))
...
func (s *Server) registerCustomerRoutes(mux *http.ServeMux) {
	...
	mux.HandleFunc("GET /api/payments/notify/{provider}", s.handlePaymentNotify)
	mux.HandleFunc("POST /api/payments/notify/{provider}", s.handlePaymentNotify)
