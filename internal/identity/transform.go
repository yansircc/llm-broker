...
// SetProfileSeed sets the server-wide seed used to derive canonical profiles.
func (t *Transformer) SetProfileSeed(seed string) {
	t.profileSeed = seed
}
...
func (t *Transformer) Transform(
	...
	// 2. Sanitize environment leaks in system prompt (Claude Code only)
	profileKey := acct.Subject
	if profileKey == "" {
		profileKey = acct.ID
	}
	profile := DeriveCanonicalProfile(profileKey, t.profileSeed)
	if looksLikeClaudeCodePrompt(body) {
		t.sanitizePromptEnv(body, profile)
	}

	// 3. Enforce cache_control compliance (max N blocks, strip TTL)
	t.enforceCacheControl(body)

	// 4. Rewrite or inject metadata.user_id
	...
	// 7. Override stainless os/arch to match canonical profile
	if result.Headers.Get("X-Stainless-Os") != "" {
		result.Headers.Set("X-Stainless-Os", profile.StainlessOS)
	}
	if result.Headers.Get("X-Stainless-Arch") != "" {
		result.Headers.Set("X-Stainless-Arch", profile.StainlessArch)
	}

	return result, nil
}

// looksLikeClaudeCodePrompt checks if the system prompt was generated by
// Claude Code (contains the canonical signature). Only Claude Code requests
// should have their <env> blocks sanitized — raw API calls are left untouched.
func looksLikeClaudeCodePrompt(body map[string]interface{}) bool {
	switch s := body["system"].(type) {
	case string:
		return strings.Contains(s, "You are Claude Code")
	case []interface{}:
		for _, entry := range s {
...
func (t *Transformer) sanitizePromptEnv(body map[string]interface{}, profile CanonicalProfile) {
	// Only sanitize system prompt blocks that contain <env> tags (injected by
	// Claude Code). User-authored system blocks are left untouched even if they
	// happen to use <env> XML — we only touch blocks with the env fingerprint
	// pattern (Platform:/Shell:/OS Version:).
	switch s := body["system"].(type) {
	case string:
		if strings.Contains(s, "<env>") {
			body["system"] = SanitizePromptEnv(s, profile)
		}
	case []interface{}:
		for _, entry := range s {
			if m, ok := entry.(map[string]interface{}); ok {
				if text, ok := m["text"].(string); ok {
					if strings.Contains(text, "<env>") && strings.Contains(text, "Platform:") {
						m["text"] = SanitizePromptEnv(text, profile)
					}
				}
			}
		}
