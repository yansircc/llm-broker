...
{:else}
	<div class="page-header">
		<div>
			...
			<h1>Control Board</h1>
			<p class="lede">Broker health, egress cells, relay outcomes, risk windows, and recent failure evidence.</p>
		</div>
	...
	<div class="metric-grid">
		<MetricCard label="cells" value={cells.length} sub={`${cells.filter((cell) => activeCooldownUntil(cell)).length} cooling`} />
		<MetricCard label="accounts" value={data.accounts.length} sub={`${activeAccounts()} active`} />
		<MetricCard label="native pool" value={availableNativeAccounts()} sub="available accounts" />
		<MetricCard label="compat pool" value={availableCompatAccounts()} sub={`${data.accounts.filter((acct) => !acct.cell_id).length} legacy direct`} />
	</div>
